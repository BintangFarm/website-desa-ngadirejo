...
import { Menu, X, MapPin, Phone, Mail, ChevronDown, User, LogOut, Shield } from "lucide-react";
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl">
                  {role === 'admin' ? <Shield className="w-4 h-4 text-slate-700" /> : <User className="w-4 h-4 text-emerald-600" />}
                  <span className="text-sm font-semibold text-slate-700">{username}</span>
                </div>
                {role === 'admin' && (
                  <Link to="/admin" className="text-xs font-semibold text-slate-600 hover:text-emerald-700 px-3 py-2 rounded-xl hover:bg-emerald-50 transition-colors">
                    Panel Admin
                  </Link>
                )}
                <button
                  ...
                  title="Keluar"
                  className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-red-600 px-3 py-2 rounded-xl hover:bg-red-50 transition-colors"
                >
            ...
            {isAuthenticated ? (
              <div className="space-y-2">
                <div className="flex items-center gap-3 px-4 py-3 bg-slate-50 rounded-xl">
                  {role === 'admin' ? <Shield className="w-5 h-5 text-slate-700" /> : <User className="w-5 h-5 text-emerald-600" />}
                  <span className="font-semibold text-slate-700">{username}</span>
                  <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-medium">{role === 'admin' ? 'Admin' : 'Pengguna'}</span>
                </div>
                {role === 'admin' && (
                  <Link to="/admin" onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-xl text-base font-semibold text-slate-700 hover:bg-slate-100 transition-colors">
                    Panel Admin
                  </Link>
                )}
                <button
                  onClick={() => { logout(); navigate('/'); setMobileMenuOpen(false); }}
                  className="w-full flex items-center gap-2 px-4 py-3 rounded-xl text-base font-semibold text-red-600 hover:bg-red-50 transition-colors"
                >
              ...
              <Link
                to="/login"
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-4 py-3.5 rounded-xl transition-colors"
              >
